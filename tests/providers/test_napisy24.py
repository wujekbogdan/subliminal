...
def test_hash_response_reads_a_catalogue_subtitle() -> None:
    archive = zip_archive(('Man.Of.Steel.2013.720p.BRRip.x264.AC3-EVO.srt', b'1\n00:00:49,591 --> 00:00:53,011\nx\n'))
...
def test_hash_response_is_none_when_no_subtitle_follows(name: str) -> None:
    assert HashResponse.from_response(hash_response(name)) is None


def test_hash_response_refuses_a_bad_account() -> None:
...
def test_hash_response_refuses_an_unreadable_response(content: bytes) -> None:
    with pytest.raises(ProviderError):
        HashResponse.from_response(content)


def test_read_archive_picks_the_subtitle_and_not_the_shortcut() -> None:
    subtitle = b'1\n00:00:49,591 --> 00:00:53,011\nx\n'
    archive = zip_archive(
...
def test_read_archive_is_none_when_no_member_is_a_subtitle() -> None:
    archive = zip_archive(('Napisy24.pl.url', b'[InternetShortcut]\nURL=http://napisy24.pl/\n'))

    assert read_archive(archive) is None


def test_read_archive_refuses_bytes_that_are_not_an_archive() -> None:
    with pytest.raises(ProviderError):
        read_archive(b'this is not a ZIP archive')


def test_catalogue_reads_a_movie_record() -> None:
...
def test_catalogue_reads_an_episode_record() -> None:
...
def test_catalogue_reads_a_response_with_a_bare_ampersand() -> None:
...
def test_catalogue_is_empty_when_the_service_found_nothing() -> None:
    assert CatalogueResponse.from_response(b'brak wynikow').records == ()


def test_catalogue_drops_a_damaged_record_and_keeps_the_others() -> None:
...
def test_catalogue_refuses_an_unreadable_response() -> None:
    with pytest.raises(ProviderError):
